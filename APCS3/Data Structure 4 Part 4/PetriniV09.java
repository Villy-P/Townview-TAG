...
public class PetriniV09 {
    public static void main(String[] args) throws IOException {
        ...
        PetriniTree<String> balanced = list.createBalancedTree();
        System.out.println(balanced);
        balanced.BreathFirstPrint();
        System.out.println("This tree has " + balanced.height() + " levels.\n");
        s.close();
    }
}
...
class PetriniTree<E extends Comparable<E>> {
    ...
    public void inorder(ArrayList<E> tree, PetriniNode<E> temp) {
        if (temp == null)
            return;
        this.inorder(tree, temp.getLeft());
        tree.add(temp.getValue());
        this.inorder(tree, temp.getRight());
    }
    ...
    public int height(PetriniNode<E> current, int height) {
        if (current == null)
            return height;
        height++;
    ...
    public PetriniTree<E> createBalancedTree() {
        PetriniTree<E> e = new PetriniTree<E>();
        ArrayList<E> inorder = new ArrayList<E>();
        this.inorder(inorder, this.root);

        int newRoot = this.getCenter(inorder);
        this.addToBalancedTree(e, inorder, newRoot);

        return e;
    }

    public void addToBalancedTree(PetriniTree<E> tree, ArrayList<E> arr, int oldIndex) {
        if (arr.size() == 0)
            return;
        tree.add(arr.get(oldIndex));
        ArrayList<E> first = this.splice(arr, 0, oldIndex);
        ArrayList<E> last = this.splice(arr, oldIndex + 1, arr.size());
        this.addToBalancedTree(tree, first, this.getCenter(first));
        this.addToBalancedTree(tree, last, this.getCenter(last));
    }

    public ArrayList<E> splice(ArrayList<E> old, int start, int end) {
    ...
    public int getCenter(ArrayList<E> e) {
        return e.size() / 2;
    }
}
